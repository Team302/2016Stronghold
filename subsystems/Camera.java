...
public class Camera {
	...
	private final int m_MinimumHue = 95;//95
	private final int m_MaximumHue = 133;//133
	private final int m_MinimumSaturation = 250;//250
	private final int m_MaximumSaturation = 360;//360
	private final int m_MinimumValue = 200;
	private final int m_MaximumValue = 360;
	
	private final double m_TargetRatio = 0.6;
	...
	private Camera(String name)
	{
        ...
        try
        {
            ...
            SmartDashboard.putString("Camera Status: ", "good Camera");
        }
        catch(Exception e)
        {
            SmartDashboard.putString("Camera Status: ", "bad Camera");
        }
	}
	...
	/**
	 * Call this to get the default camera object.
	 * This defaults to a camera name of "cam0"
	 * This is a singleton class
	 * 
	 * @return		the camera object
	 */
	...
	public void getAndSetImage()
	{
		...
		if(frame != null) CameraServer.getInstance().setImage(frame);
	}
...
//				particles.add(data);
			...
			if((ratio / m_TargetRatio) <= 1.2 && (ratio / m_TargetRatio) >= 0.7 && (right - left) < 150 && (right - left) > 50 && data[Data.Y.ordinal()] > 240)
			{
...
//		SmartDashboard.putNumber("Good Particles", goodParticles.size());
		...
		if(goodParticles.size() != 0)
		{
			...
			double ratio = getTargetProportionalityScore(goodParticles.get(bestTargetIndex)[Data.HEIGHT.ordinal()], goodParticles.get(bestTargetIndex)[Data.WIDTH.ordinal()]);
...
//			SmartDashboard.putNumber("right: ", goodParticles.get(bestTargetIndex)[Data.RIGHT.ordinal()]);
			...
			NIVision.imaqDrawShapeOnImage(frame, frame, center, DrawMode.DRAW_VALUE, ShapeMode.SHAPE_RECT, 1.0f);
			NIVision.imaqDrawShapeOnImage(frame, frame, boundingRectangle, DrawMode.DRAW_VALUE, ShapeMode.SHAPE_RECT, 1.0f);
...
//		SmartDashboard.putNumber("Y: ", m_TargetY);
	 ...
	 private void drawCrosshairs()
	 {
	     if(frame != null)
	     {
	         NIVision.imaqDrawLineOnImage(frame, frame, DrawMode.DRAW_VALUE, new Point(342-50, 358), new Point(342+50, 358), 1.0f);
	         NIVision.imaqDrawLineOnImage(frame, frame, DrawMode.DRAW_VALUE, new Point(342, 358-50), new Point(342, 358+50), 1.0f);
	     }
	 }
